...
Template.Students_Summary_Widget.onCreated(function studentsSummaryWidgetOnRendered() {
  ...
  this.totalSessions = new ReactiveVar(0);
...
Template.Students_Summary_Widget.helpers({
  ...
  dateRange() {
    const dateRange = Template.instance()
      .dateRange
      .get();
  ...
  uniqueVisitors() {
    const uniqueVisitors = Template.instance()
      .unique
      .get();
    if (uniqueVisitors > 0) {
      return `${uniqueVisitors} Unique Visitors,`;
    }
    return '';
  },
  enrolledStudents() {
    const enrolledStudents = Template.instance()
      .enrolled
      .get();
    if (enrolledStudents > 0) {
      return `${enrolledStudents} Enrolled Students`;
    }
    return '';
  },
  totalSessions() {
    const totalSessions = Template.instance()
      .totalSessions
      .get();
    if (totalSessions > 0) {
  ...
  selectedUser() {
    return !_.isEmpty(Template.instance()
      .selectedUser
      .get());
  },
  renderedSummary() {
  ...
  timelineChart() {
    const interactionsByUser = Template.instance()
      .interactionsByUser
      .get();
    const enrolledStudents = Template.instance()
      .enrolled
      ...
      _.each(behaviorsByDate, function (array, date, obj) {
        _.each(interactionsByUser, function (interactions) {
          const interactionsWithinDate = _.filter(interactions, function (interaction) {
            const interactionDate = moment(interaction.timestamp)
              .format('MMM D, YYYY');
...
Template.Students_Summary_Widget.events({
  submit(event, instance) {
    ...
    const startDate = moment(event.target.startDate.value, 'MMMM D, YYYY')
      .toDate();
    const endDate = moment(event.target.endDate.value, 'MMMM D, YYYY')
      .endOf('day')
      .toDate();
    const dateRange = { startDate, endDate };
    const startTerm = Semesters.getSemesterDoc(startDate);
    const endTerm = Semesters.getSemesterDoc(endDate);
    const semesters = Semesters.find({
      semesterNumber: { $gte: startTerm.semesterNumber, $lte: endTerm.semesterNumber },
    })
      .fetch();
    let courseInstances = [];
    _.forEach(semesters, (s) => {
      courseInstances = courseInstances.concat(CourseInstances.find({ semesterID: s._id })
        .fetch());
    });
