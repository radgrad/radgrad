...
class StudentProfileCollection extends BaseProfileCollection {
  ...
  dumpOne(docID) {
    ...
    const hiddenOpportunities = _.map(doc.hiddenOpportunityIDs, hiddenOpportunityID => Opportunities.findSlugByID(hiddenOpportunityID)); // eslint-disable-line max-len
    const { isAlumni } = doc;
    const { retired } = doc;
